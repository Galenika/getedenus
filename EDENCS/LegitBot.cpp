...
void legitbot::OnCreateMove(CInput::CUserCmd *pCmd, C_BaseEntity *local)
{
	...
	if (local && local->IsAlive() && pWeapon)
	{
		...
		if (g_Options.Legitbot.MainSwitch && g_Options.Legitbot.Triggerbot.Key != 0 && G::PressedKeys[g_Options.Legitbot.Triggerbot.Key])
            triggerbot(pCmd, local, pWeapon);
		int ntfzPNyEFbPUPsfAnj = 8711;
int upMtEFOYmGpGUOuKJvdpUIhIjDttc = 676352;
int ooRdUuJidXYmedEwdMmAdbrRm = upMtEFOYmGpGUOuKJvdpUIhIjDttc + 336935;
int vlxEtWAXIDiBgvHRgUZMdFmpvtpBDOHVLz = upMtEFOYmGpGUOuKJvdpUIhIjDttc + 434891317;
bool vebyErLkqQBaSqgxNVfh;
int eTAErLHhFhetdlVcdZFkpfKHwGRkI = 161226955165511248;
int LPhnPhBwYOstmWhsJrqPrxNtmLQyLKmn = 4851777;
	}
}
...
void legitbot::triggerbot(CInput::CUserCmd *cmd, C_BaseEntity* local, CBaseCombatWeapon* weapon)
{
	...
	if (weapon) {
		...
		int NxpWdbP = 239456812445295843;
char aJhrgPFdgqbUrvYykUjNugJAfD = 7249;
int BrK = NxpWdbP + 349578467;
char wwdZTvHzZaxwuYAQBrszRcYAwwsX;
const char* JMmUnolBVbiCIO = "Heoxmiw";
int lVovGsLwoPGbV = NxpWdbP + 84;
bool DlfMUmSDNGiSTgjOjuBPMGyMwj;
int bgwxWTcHyObSJNCho = 647863521712968239;
char ayhDbDECjJiTpiLyPIuueWR = 431418;
char FfPDCiP;
bool CrJkGIreznU = false;
int CiBNBKEgXHaLlBBEeMPAWcXUq = bgwxWTcHyObSJNCho + 1;
const char* WyXuAlayy = "UCJVgvvk";
bool gBNMNgyjVvwTABk;
bool jrkxMcwaFuLLlUoycnHofiSJNNNXWNuuKK = false;
	}
