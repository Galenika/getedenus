...
void __fastcall hkPaintTraverse(PVOID pPanels, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	else if (FocusOverlayPanel == vguiPanel)
	{
		...
		if (g_Engine->IsConnected() && g_Engine->IsInGame())
		{
			...
			int height = 0;
			g_Engine->GetScreenSize(width, height);
			g_Render->DrawRect(width - 210, 4, 182 + width - 187, 20, Color(0, 161, 255, 85)); //  Color(175, 85, 107, 255)
			g_Render->Text(width - 210 + 4, 5, Color(255, 255, 255, 255), g_Render->font.ESP, XorStr("geteden.us | build date: 6/6/18 | Beta"));
		...
		{
			static bool* s_bOverridePostProcessingDisable = *(bool**)(U::pattern_scan(GetModuleHandleW(L"client.dll"), "80 3D ? ? ? ? ? 53 56 57 0F 85") + 0x2);
