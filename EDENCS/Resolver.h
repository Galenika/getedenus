	...
	if (g_Options.Ragebot.Resolver)
	{
		C_BaseEntity *pLocal = g_EntityList->GetClientEntity(g_Engine->GetLocalPlayer());
		{
			for (auto i = 0; i < g_EntityList->GetHighestEntityIndex(); i++)
			{
				...
				if (entity->GetVelocity().Length2D() <= 0) // yes, i know your velocity cant be negative atleast i think it just makes our codenz look cool!!!!!!!!! !!!
				{
					isStationary = true;
